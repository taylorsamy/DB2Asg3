...
def validate_department_data(df):
    ...
    if df_copy['Department_ID'].is_unique == False:
        ...
        invalid_ID_department_df = pd.concat([invalid_ID_department_df, df_copy[df_copy['Department_ID'].duplicated(keep=False)]])
    ...
    if df_copy['Department_Name'].is_unique == False:
        ...
        invalid_name_department_df = pd.concat([invalid_name_department_df, df_copy[df_copy['Department_Name'].duplicated(keep=False)]])
    ...
    if df_copy['DOE'].dtypes == 'object':
        ...
        invalid_DOE_department_df = pd.concat([invalid_DOE_department_df, df_copy[df_copy['DOE'].isnull()]])
    ...
    if df_copy['DOE'].dt.year.lt(1900).any() == True:
        ...
        invalid_DOE_department_df = pd.concat([invalid_DOE_department_df, df_copy[df['DOE'].dt.year < 1900]])
    ...
    if df_copy.isnull().values.any() == True:
        ...
        invalid_NULL_department_df = pd.concat([invalid_NULL_department_df, df_copy[df_copy.isnull().any(axis=1)]])
    ...
    invalid_department_df = pd.concat([invalid_ID_department_df, invalid_name_department_df, invalid_DOE_department_df, invalid_NULL_department_df])
...
def validate_student_data(df, valid_department_df):
    ...
    if df_copy['Department_Admission'].isnull().values.any() == True:
        # if there are null values, remove the all of the rows where there are null values from the dataframe and add it to the invalid dataframe using the concat method
        invalid_NULL_student_df = pd.concat([invalid_NULL_student_df, df_copy[df_copy['Department_Admission'].isnull()]])
    ...
    if df_copy['Department_Admission'].isin(valid_department_df['Department_ID']).all() == False:
        ...
        invalid_student_df = pd.concat([invalid_student_df, df_copy[~df_copy['Department_Admission'].isin(valid_department_df['Department_ID'])]])
...
def validate_performance_data(df, valid_student_df):
    ...
    if df_copy['Marks'].dtypes == 'object':
        ...
        invalid_marks_num_performance_df = pd.concat([invalid_marks_num_performance_df, df_copy[df_copy['Marks'].isnull()]])
    ...
    if df_copy['Marks'].lt(0).any() == True:
        ...
        invalid_marks_0_performance_df = pd.concat([invalid_marks_0_performance_df, df_copy[df_copy['Marks'] < 0]])
    ...
    if df_copy['Marks'].gt(100).any() == True:
        ...
        invalid_marks_100_performance_df = pd.concat([invalid_marks_100_performance_df, df_copy[df_copy['Marks'] > 100]])
    ...
    if df_copy['Effort_Hours'].dtypes == 'object':
        ...
        df_copy['Effort_Hours'] = pd.to_numeric(df_copy['Effort_Hours'], errors='coerce')
        ...
        invalid_hours_num_performance_df = pd.concat([invalid_hours_num_performance_df, df_copy[df_copy['Effort_Hours'].isnull()]])
    ...
    if df_copy['Effort_Hours'].lt(0).any() == True:
        ...
        invalid_hours_min_performance_df = pd.concat([invalid_hours_min_performance_df, df_copy[df_copy['Effort_Hours'] < 0]])
    ...
    if df_copy.duplicated(subset=['Student_ID', 'Paper_ID']).any() == True:
        ...
        invalid_paper_performance_df = pd.concat([invalid_paper_performance_df, df_copy[df_copy.duplicated(subset=['Student_ID', 'Paper_ID'])]])
    ...
    if df_copy.isnull().values.any() == True:
        # if there are NULL values, remove the all of the rows where there are NULL values from the dataframe and add it to the invalid dataframe using the concat method
        invalid_NULL_performance_df = pd.concat([invalid_NULL_performance_df, df_copy[df_copy.isnull().any(axis=1)]])
        df_copy = df_copy.dropna()
    ...
    invalid_marks_performance_df = pd.concat([invalid_marks_0_performance_df, invalid_marks_100_performance_df, invalid_marks_num_performance_df, invalid_hours_num_performance_df, invalid_hours_min_performance_df, invalid_paper_performance_df, invalid_NULL_performance_df])
